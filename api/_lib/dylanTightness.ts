...
function statusFor(player: SteamPlayer) {
  ...
  if (elden) return { online, apex, elden, tightness: 96, statusLabel: "really really tight" }
  if (apex) return { online, apex, elden, tightness: 72, statusLabel: "tighter" }
  if (online) return { online, apex, elden, tightness: 38, statusLabel: "slightly tight" }
  return { online, apex, elden, tightness: 8, statusLabel: "hella loose" }
}
...
async function readLastSeen(column: "is_online" | "is_apex" | "is_elden") {
  if (!supabaseConfig()) {
...
async function getStoredState() {
  try {
    const [history, latest, online, apex, elden] = await Promise.all([
      readHistory(),
      readLatestSample(),
    ...
    return {
      ...
      latest: null,
      lastSeen: { online: null, apex: null, elden: null },
